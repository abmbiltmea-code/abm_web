...
//           <div className="relative w-full aspect-849/559 shrink-0 lg:w-[50%] 3xl:w-[849px] h-[217px] sm:h-auto max-lg:max-h-[350px] lg:h-auto 3xl:h-[559px] mr-50 xl:mr-70 3xl:mr-[76px] order-2 lg:order-1">
//             {items.map((item, i) => (
//               <div
//                 key={i}
...
export default function Evolution({ data }: { data: FifthSection }) {
  ...
  useLayoutEffect(() => {
    const measureActiveLeft = () => {
      const btn = mobileButtonRefs.current[displayIndex];
      if (btn) setActiveLeft(btn.offsetLeft);
    };
    measureActiveLeft();
    window.addEventListener("resize", measureActiveLeft);
    return () => window.removeEventListener("resize", measureActiveLeft);
  }, [displayIndex]);
  ...
  const shiftX = -activeLeft + dragOffset;
      ...
      <div className="container">
        ...
        >
          ...
          >
            <div
              ref={listRefMobile}
              className={`absolute top-0 bottom-0 left-0 flex flex-row items-center select-none ${
                transitionEnabled
                  ? "transition-transform duration-500 ease-in-out"
                  : ""
              }`}
...
className={`shrink-0 whitespace-nowrap text-center transition-colors duration-300 h-[30px] text-subtitle font-bold font-tasa leading-none cursor-pointer rounded-[30px] flex items-center px-[21px] py-[5px] border ${
                      isActive
                        ? "border-white text-primary bg-[#D9D9D917]"
