...
const FormFileInput = forwardRef<HTMLInputElement, FormFileInputProps>(
  (
    { label, name, required = false, error, fileName, onFocus, ...rest },
    ref,
  ) => {
    const autoId = useId();
    ...
    return (
      <div className="relative w-full pb-[calc(25px+20px)] md:pb-[calc(40px+20px)] cursor-pointer">
        <input
          id={inputId}
          type="file"
          name={name}
        ...
        cursor-pointer text-description-color select-none
        text-description
        ${
          floated
