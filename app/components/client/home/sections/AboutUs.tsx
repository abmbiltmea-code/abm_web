...
import { SecondSection } from "@/app/types/home";

export default function AboutSection({data} : {data : SecondSection}) {
  const { sectionLabel, title, description, icon, iconAlt, items } = data;
    ...
    <section className="w-full bg-cream-background py-120 3xl:py-140 overflow-hidden">
      <div className="container flex flex-col lg:flex-row 3xl:justify-between gap-y-5 md:gap-y-[30px]">
        ...
        <div className="shrink-0">
          <SectionLabel title={sectionLabel} pt="lg:pt-[17px]" />
        </div>
        ...
        <div className="flex flex-col lg:section-content-spacing">
          ...
          <div className="flex justify-between gap-40 w-full">
            {items.map((item, i) => (
              ...
              whileInView="show"
              viewport={{ once: true }}
              key={item.label} className="flex flex-col gap-[5px] md:gap-[10px]">
                <div className="flex items-center gap-1.5 lg:gap-3 3xl:gap-6 font-tasa">
                  <span className="text-100 font-semibold text-secondary leading-none md:leading-[1.2]">
                    <AnimatedCounter from={Number(item.value) - 8} to={item.value} duration={1.6} />
                  </span>
